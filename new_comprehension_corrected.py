...
def multiply_matrices(matA, matB):
    ...
    result = [[sum(a * b 
            for a, b in zip(rows_in_A, cols_in_B))
            for cols_in_B in zip(*matB)]
            for rows_in_A in matA]
...
# do this for each individual list. 
for i in mm:
    sys.stdout.write((display_dot_product(i)))
...
for line in infile:
    ...
    if line.startswith("Hsa"): # parse the file piece by piece
        ...
        # Access the log intensity values based on whether the category is Cancer or Control. 
